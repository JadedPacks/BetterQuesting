...
public class GuiJsonArray extends GuiScreenThemed implements IVolatileScreen {
	...
	private ArrayList<JsonControlSet> editables = new ArrayList<>();
	...
	public void initGui() {
		...
		for(int i = 0; i < settings.size(); i++) {
			...
			if(entry.isJsonPrimitive()) {
				...
				GuiButtonJson<JsonElement> button = new GuiButtonJson<>(buttonList.size(), -9999, -9999, 128, 20, entry, true);
				editables.add(new JsonControlSet(this.buttonList, button, true, true));
				this.buttonList.add(button);
			}
		}
				...
				if(list.get(i).getAsJsonPrimitive().isNumber()) {
					if(textField instanceof GuiNumberField) {
						...
						try {
							...
							BetterQuesting.logger.error("Unable to parse number format for JsonArray!", e);
