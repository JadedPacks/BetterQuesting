...
public class GuiPrerequisiteEditor extends GuiScreenThemed implements IVolatileScreen, INeedsRefresh {
	...
	public void refreshGui() {
		QuestInstance tmp = QuestDatabase.getValue(questID);
		if(tmp == null) {
			mc.displayGuiScreen(parent);
			return;
		}
	...
	public void RefreshColumns() {
		...
		for(QuestInstance prq : quest.getPrerequisites()) {
			int qID = QuestDatabase.getKey(prq),
				btnWidth = prBtnList.getListWidth(),
				bID = (2 + qID) << 3;
			GuiButtonThemed btn1 = new GuiButtonThemed(bID, 0, 0, btnWidth - 20, 20, I18n.format(prq.name));
		...
		for(int qID : searchResults) {
			QuestInstance dbQ = QuestDatabase.getValue(qID);
			if(dbQ == null) {
				continue;
			}
			...
			btn3.enabled = quest != dbQ && !quest.getPrerequisites().contains(dbQ);
			GuiButtonThemed btn4 = new GuiButtonThemed(bID + 3, 0, 0, bWidth - 40, 20, I18n.format(dbQ.name));
